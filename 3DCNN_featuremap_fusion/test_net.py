    ...
    for i in range(num_images):
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            if inds.numel() > 0:
                ...
                if vis:
                    im2show = vis_detections(im2show, imdb.classes[j], cls_dets.cpu().numpy(), roidb[i], 0.8)
...
def testing_fusion():
    # initilize the network here.
    if args.net == 'vgg16f':
        fasterRCNN = vgg16f(imdb.classes, pretrained=False, class_agnostic=args.class_agnostic, fusion_mode="half")

    elif args.net == 'vgg16c':
        fasterRCNN = vgg16c(imdb.classes, pretrained=False, class_agnostic=args.class_agnostic)
    elif args.net == 'res101':
        fasterRCNN = resnet(imdb.classes, 101, pretrained=False, class_agnostic=args.class_agnostic)
    elif args.net == 'res50':
        fasterRCNN = resnet(imdb.classes, 50, pretrained=False, class_agnostic=args.class_agnostic)
    ...
    if args.net == 'vgg16f':
        for i in range(num_images):
            ...
            data = next(data_iter)
            im_data1.data.resize_(data[0].size()).copy_(data[0])
            im_data2.data.resize_(data[1].size()).copy_(data[1])
            ...
            for j in xrange(1, imdb.num_classes):
                ...
                if inds.numel() > 0:
                    ...
                    cls_dets = cls_dets[keep.view(-1).long()]
                    #print('hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh',cls_dets.cpu().numpy().shape[0])
                    if vis:
                        im2show = vis_detections(im2show, imdb.classes[j], cls_dets.cpu().numpy(), roidb[i], 0.7)
                    all_boxes[j][i] = cls_dets.cpu().numpy()
                else:
                    all_boxes[j][i] = empty_array
    ...
    if args.net == 'vgg16c':
        for i in range(num_images):
            ...
            pred_boxes /= data[2][0][2].item()
            #print('hhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhhh',pred_boxes)
...
if __name__ == '__main__':
  ...
  if args.dataset == "kaist_test-all20-rgb":
      args.imdb_name = "kaist_test-all20-rgb"
      args.imdbval_name = "kaist_test-all20-rgb"
      args.set_cfgs = ['ANCHOR_SCALES', '[8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '20']

  elif args.dataset == "kaist_test-all20-fusion":
      args.imdb_name = "kaist_test-all20-fusion"
      args.imdbval_name = "kaist_test-all20-fusion"
      #args.set_cfgs = ['ANCHOR_SCALES', '[8, 16, 24]', 'ANCHOR_RATIOS', '[2,2.3,2.6]', 'MAX_NUM_GT_BOXES', '20']
      args.set_cfgs = ['ANCHOR_SCALES', '[8, 16, 32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '20']
  ...
  ############################# output path ########################################
  save_name = args.model_dir + '_' + cfg.POOLING_MODE + '_' + 'faster_rcnn_10' + '_' + str(args.checkepoch) + '_' + str(
      args.checkpoint)
  output_dir = get_output_dir(imdb.name, save_name)

  log_output = output_dir + '/test.log'
  Log = Logger.getLogger("train.log")
  Log.info(cfg)
  Log.info(args)

  ############################# model path  #########################################
  input_dir = args.load_dir + "/" + args.net + "/" + args.model_dir
