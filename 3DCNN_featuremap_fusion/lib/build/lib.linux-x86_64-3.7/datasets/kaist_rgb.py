...
class kaist_rgb(imdb):
    ...
    def __init__(self, image_set, devkit_path=None):
        imdb.__init__(self, 'kaist_'+image_set)  # image_set: train-all02 or test-all
    ...
    def image_path_from_index(self, index):
        ...
        image_path = os.path.join(self._data_path, self._image_set, 'images',
                                  index[:-6] + self._image_type + index[-7:] + self._image_ext)
    ...
    def gt_roidb(self):
        ...
        #gt_roidb = [self._load_kaist_annotation_test(index)
        #            for index in self.image_index]
    ...
    def _load_kaist_annotation_test(self, index):
        """
        Load image and bounding boxes info from text file in the kaist dataset
        format.
        """
        ...
        new_index = '_'.join(index.split('/'))
        new_gtfilename = os.path.join(self._data_path, self._image_set, 'annotations_KAIST_test_set',
                                      new_index + '.txt')
        ...
        boxes2 = np.zeros((num_pers, 4), dtype=np.uint16)
