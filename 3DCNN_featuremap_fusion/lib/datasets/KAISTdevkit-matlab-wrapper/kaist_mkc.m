...
for ie = 1:9
    res = run_exp(res, exps(ie,:), exps(ie,2), gtDir, bbsNms);
end
...
% bbsNm.test-all
for cond = [{'test-all20'}, {'test-day'}, {'test-night'}]
    ...
    for s=1:length(setIds)
        for v=1:length(vidIds{s})
            for i=skip-1:skip:99999
                detName = sprintf('set%02d/V%03d/I%05d.txt', setIds(s), vidIds{s}(v), i);
...
function res = run_exp(res, iexp, testSet, gtDir, bbsNms)
...
testSet = fullfile(gtDir,testSet{1}, testSetName);
...
[gt,dt] = bbGt2('loadAll',annoDir,bbsNm,pLoad,testSet);
